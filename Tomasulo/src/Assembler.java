	...
	public ArrayList<InstructionEntry> read() {
		...
		try{
			...
			while( (s = br.readLine()) != null){
				...
				if(st.hasMoreTokens()) inst.setRD(Integer.parseInt(st.nextToken()));
				if(st.hasMoreTokens()) inst.setRS(Integer.parseInt(st.nextToken()));
				if(st.hasMoreTokens()) inst.setRT(Integer.parseInt(st.nextToken()));
